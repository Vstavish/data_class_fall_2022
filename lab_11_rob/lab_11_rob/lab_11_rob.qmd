...
Sys.setenv(SPOTIFY_CLIENT_ID = 'e0171c5588fb49caa1abd0859a873b72')
Sys.setenv(SPOTIFY_CLIENT_SECRET = '0e31156ed6c04a3ab76a94e1343edfbf')
...
# Q1: how does the danceability of Taylor Swift's music change over time?
# Q2: What is the correlation between tempo and song length in Taylor Swift's music
...
```{r}
#Q1: how does the danceability of Taylor Swift's music change over time?
#A1: Taylor Swift's music has gotten less danceable since 2020

danceability_over_time <- taylor_swift_discography %>%
  as.data.frame() %>%
  clean_names() %>%
  select(danceability, album_release_year)

danceability_over_time %>%
ggplot() +
  geom_bar(aes(x=album_release_year, weight=danceability))


#Q2: What is the correlation between tempo and song length in Taylor Swift's music
#A1: There is a very loose correlation between Taylor Swift's song length and tempo. It seems that low tempo songs may actually be shorter than tracks with higher tempos. 

temp_length <- taylor_swift_discography %>%
  as.data.frame() %>%
  clean_names() %>%
  select(tempo, duration_ms)

ggplot(temp_length, aes(y=tempo, x=duration_ms)) +
geom_point() +
  labs(
    title="Are Taylor Swift's sadder songs longer?",
    subtitle = "Not really ",
    x = "song length",
    y = "tempo",
    caption = "Source: Spotify API  By: Victoria Stavish")
