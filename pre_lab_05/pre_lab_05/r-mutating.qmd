...
pct_change <- city_md_income %>%
  mutate(pctchange = (median_inc_2020 - median_inc_2010)/(median_inc_2010)*100)
...
```{r}
# highest pct_change was in Shaft

  pct_change %>%
    slice_max(pctchange, n = 10)

# absolute change was highest in Beaver Creek
absolute_change <- city_md_income %>%
...
```{r}
bottom_25 <- pct_change %>%
  slice_min(pctchange, n=25)
...
Look at the summary statistics of the column and then whatever the median is, filter for that number in the percentage change column.

to look at a table with top 10 and bottom 10 pct change I would create two new tables from a slice max and slice min and then join those two new tables.
