...
```{r}
baltcity_income %>% 
  dplyr::select(Neighborhood, x2010, x2016, x2020, Census) %>% 
  summarise(
    count_tracts = n(),
...
```{r}
baltcity_income %>%
  dplyr::select(Neighborhood, x2020, Census) %>% 
    summarise(
    count_tracts = n(),
...
```{r}
baltcity_race %>% 
  dplyr::select(x2020_white, x2020_black) %>% 
  summarize(
...
# 49875
# all census tracts below median income 2020
below_median <- baltcity_income %>%
  filter(x2020 < 49875, na.rm=TRUE)

# percent of tracts that are below median income 2020 = 48.5%
percent_below <- (97/200)*100

# tweet
# About 49% of neighborhoods in Baltimore cities have a median income less than the Baltimore City median of $49,875, according to Census data.
